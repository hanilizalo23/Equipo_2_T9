...
void LCD_nokia_bitmap(const uint8_t bitmap [], uint16_t Length)
{
	...
	GPIO_PortSet(GPIOD, LCD_DATA << DATA_OR_CMD_PIN);

	/*Start master transfer, send data to slave, not receiving from Slave*/
...
void LCD_nokia_write_byte(uint8_t data_or_command, uint8_t data)
{
	...
	if(data_or_command)
		...
		GPIO_PortClear(GPIOD, LCD_DATA << DATA_OR_CMD_PIN);
...
void LCD_nokia_send_char(uint8_t character) {
	...
	for (index = 0 ; index < RESET_PIN ; index++)
...
void LCD_nokia_clear(void)
{
	...
	for (index = 0 ; index < (LCD_X * LCD_Y / LENGTHXY) ; index++)
